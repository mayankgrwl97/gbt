...
from gbt.renderer.cameras import pixel_to_ndc
from gbt.utils.device import get_default_device
from gbt.utils.typing import is_scalar
...
def process_query_ray_filter(query_ray_filter):
    ...
    for b in range(B):
        idx_for_batch_b = non_zero_idx[non_zero_idx[:, 0]==b]  # (n_b, 3) --> contains tensors like [[b, y1, x1], [b, y2, x2], ... [b, yn, xn]]
...
def get_patch_rays(cameras_list, num_patches_x, num_patches_y, device):
    ...
    patch_rays = [get_patch_raybundle(camera, num_patches_y=num_patches_y, num_patches_x=num_patches_x) for camera in cameras_list]

    patch_rays = [torch.cat((patch_ray.origins, patch_ray.directions), dim=-1) for patch_ray in patch_rays]  # list of len (B * num_views), having (1, P, 6) tensors
...
def get_random_query_pixel_rays(cameras_list, num_pixel_queries, query_ray_filter, min_x,
                                min_y, max_x, max_y, return_xys=False,
    ...
    random_query_rays = [get_random_raybundle(camera, num_rays=num_pixel_queries, min_x=min_x, min_y=min_y, max_x=max_x, max_y=max_y) \
...
def get_grid_rays(cameras_list, image_size, min_x, min_y,
                  max_x, max_y, device=get_default_device()):
    ...
    grid_rays = [get_grid_raybundle(camera, image_size=image_size, min_x=min_x, min_y=min_y, max_x=max_x, max_y=max_y) for camera in
                 cameras_list]
