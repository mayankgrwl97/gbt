...
class Co3dV2Dataset(CO3Dv2Wrapper):
    ...
    def __getitem__(self, index):
        ...
        if self.num_query_views == 1:
            sparse_query_cameras = sparse_query_cameras[0]
            sparse_query_images = sparse_query_images.squeeze(0)
            sparse_query_indices = sparse_query_indices[0]
            sparse_query_crop_masks = sparse_query_crop_masks.squeeze(0)
            sparse_query_fg_masks = sparse_query_fg_masks.squeeze(0)
            sparse_query_fg_masks_soft = sparse_query_fg_masks_soft.squeeze(0)
            sparse_query_bboxes = sparse_query_bboxes.squeeze(0)

        result = {
            "obj_id": item['idx'], #scene_id,
            "path": os.path.join(self.path, self.category, item['idx']),
            "focal": item['f'], #input_cameras[0].focal_length,
            "c": item['c'], #input_cameras[0].principal_point,
            "sparse_input_images":  sparse_input_images,
            "sparse_query_images": sparse_query_images,
            "sparse_input_cameras": sparse_input_cameras,
            "sparse_query_cameras": sparse_query_cameras,
            "sparse_input_indices": sparse_input_indices,
            "sparse_input_frame_numbers": sparse_input_frame_numbers,
            "sparse_query_indices": sparse_query_indices,
            "sparse_query_frame_numbers": sparse_query_frame_numbers,
            "sparse_input_bboxes": sparse_input_bboxes,
            "sparse_query_bboxes": sparse_query_bboxes,
            "sparse_input_fg_masks": sparse_input_fg_masks,
            "sparse_query_fg_masks": sparse_query_fg_masks,
            "sparse_input_crop_masks": sparse_input_crop_masks,
            "sparse_query_crop_masks": sparse_query_crop_masks,
            "sparse_input_fg_masks_soft": sparse_input_fg_masks_soft,
            "sparse_query_fg_masks_soft": sparse_query_fg_masks_soft
        }
